...
/**
 * @brief Ethernet ve IP olay işleyicisi
 */
                              ...
                              int32_t event_id, void *event_data)
{
    ...
    if (event_base == ETH_EVENT) {
        ...
        case ETHERNET_EVENT_CONNECTED:
            // MAC adresini al - handle'ı global değişkenden kullan
...
/**
 * @brief Ağ hizmetlerini (Event Handler'ları) kaydeder.
 */
esp_err_t register_eth_service(void)
{
    // Ethernet olay işleyicilerini kaydet
    ESP_ERROR_CHECK(esp_event_handler_register(ETH_EVENT, ESP_EVENT_ANY_ID, &eth_event_handler, NULL));
    // IP olayı işleyicisini kaydet
...
/**
 * @brief Ethernet handle'ı sakla (ETHERNET_EVENT_CONNECTED için gerekli)
 */
