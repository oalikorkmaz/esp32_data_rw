    ...
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0) {
                ...
                ESP_LOGI(TAG, "Bağlantı başarısız; yeniden reklam başlatılıyor...");
...
esp_err_t ble_cfg_init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "NimBLE init hatası: %s", esp_err_to_name(ret));
        return ret;
    }
